...
exports.run = async(client, message, args) => {
  ...
  if (!args[0]) return message.channel.send(`:no_entry: Reklam Filtresini Ayarlamak İçin \`m!link aç\` | Kapatmak İstiyorsanız \`m!link kapat\` Yazabilirsiniz`)
  if (args[0] !== 'aç' && args[0] !== 'kapat') return message.channel.send(`:no_entry: Reklam Filtresini Ayarlamak İçin \`m!link aç\` | Kapatmak İstiyorsanız \`m!link kapat\` Yazabilirsiniz`)
