...
/////
const app = express()
app.get('/', (req, res) => res.send("Bot Aktif | Exnoim"))
...
client.on("ready", () => {
  ...
  client.user.setActivity('m!yardım | Exnoim');
})
...
db.set(`goldzzz_${msg.author.id}`, Date.now());
  ...
  .setAuthor(`👉 Premium 👈`)
  .setDescription(`Bir özel üye belirdi\n\n <@${msg.author.id}>`)
          ...
          try {
            if (!msg.member.hasPermission("BAN_MEMBERS")) {
                  msg.delete();
                      ...
                      return msg.reply('Yasaklı kelimeler kullanıyorsun yakaladım.').then(msg => msg.delete(3000));
            }              
          } catch(err) {
            console.log(err);
          }
...
client.login(ayarlar.token)
  ...
  client.on("ready", () => {
  client.channels.cache.get("").join();//sesli bi kanala katılacagı icin kanal ID si
}) 
...
.addField("Sunucudaki Kişi Sayısı:", guild.memberCount)
   ...
   client.channels.get('712742698326425672').send(rrrsembed);
